...
async function run() {
  ...
  const estsSnap = await db.collection('estabelecimentos').get();
  
  for (const estDoc of estsSnap.docs) {
    const estabId = estDoc.id;
    const estName = estDoc.data().nome;
    ...
    if (countValidos > 0 || totalVendas > 0) {
      console.log(`Establishment: "${estName}" (${estabId}) => Valid Orders: ${countValidos} | Total: R$ ${totalVendas.toFixed(2)} | Delivery: R$ ${totalTaxas.toFixed(2)}`);
    }
  }
}
