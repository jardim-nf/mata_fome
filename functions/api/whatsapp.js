...
import { buscarProdutosRobo, processarFluxoRobo, enviarWhatsAppUAZAPI } from '../services/whatsappService.js';

// Caches em memória para otimização de consultas Firebase
const whatsappInstanceCache = new Map();
const metaPageCache = new Map();
...
export const webhookWhatsApp = onRequest({ secrets: [whatsappVerifyToken, whatsappApiToken], maxInstances: 5 }, async (req, res) => {
    ...
    try {
      ...
      let estabId = whatsappInstanceCache.get(instanceId);
      ...
      if (!estabId) {
          ...
          estabId = estabQuery.docs[0].id;
          whatsappInstanceCache.set(instanceId, estabId);
      }

      const estabDoc = await db.collection('estabelecimentos').doc(estabId).get();
      if (!estabDoc.exists) return res.status(200).send('OK');
