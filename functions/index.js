...
import { getAuth } from "firebase-admin/auth"; // <-- IMPORT ADICIONADO PARA CRIAR USUÁRIOS
...
// ==================================================================
// 11. CRIAR USUÁRIO VIA MASTER ADMIN (CORRIGIDO CORS)
// ==================================================================
export const createUserByMasterAdminHttp = onCall({ 
    cors: true 
}, async (request) => {
    // 1. Opcional: Validar se quem está chamando está autenticado e é admin
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'O usuário precisa estar autenticado para realizar esta ação.');
    }

    const { email, password, displayName, role, estabelecimentoId, ...outrosDados } = request.data;
    ...
    try {
        // 3. Cria o usuário diretamente no Firebase Authentication
        ...
        logger.info(`✅ Novo usuário criado no Auth: ${userRecord.uid}`);

        // 4. Salva as informações do usuário no banco de dados Firestore
        ...
        logger.info(`✅ Documento do usuário salvo no Firestore: ${userRecord.uid}`);
        ...
        logger.error("❌ Erro ao criar usuário pelo Admin:", error);
        // Retorna o erro específico (ex: email já existe) de forma legível
