...
// 🚨 CORREÇÃO: Adicionamos a prop 'onPagar' para a finalização
...
    };
    ...
    // 🚨 NOVA FUNÇÃO DE CLIQUE PARA PAGAMENTO (EVITA ERRO DE ONCLICK)
    const handleCardClick = () => {
        if (mesa.status === 'pagamento') {
            // Se for pagamento, chamamos a função de liberar a mesa
            onPagar(mesa.id, mesa.numero);
        } else {
            // Se for livre, ocupada ou com_pedido, chamamos a função de abrir o editor
            onClick();
        }
    };
...
    return (
        <div 
            // 🚨 CORREÇÃO: Usa a nova lógica de clique
