...
// Importe 'storage' e as funções do storage de 'firebase/storage'
import { storage } from '../firebase'; // Certifique-se que 'storage' está exportado em '../firebase.js'
import { ref, uploadBytes, getDownloadURL, deleteObject } from 'firebase/storage'; // Adicione deleteObject para remover imagens antigas
import { useAuth } from '../context/AuthContext';
import { toast } from 'react-toastify';
// Importa o serviço de upload que criamos (se você o criou)
...
function AdminMenuManagement() {
    ...
    // Adicione itemImage para armazenar o objeto File e imagePreview para pré-visualização
    const [formData, setFormData] = useState({ nome: '', descricao: '', preco: '', categoria: '', imageUrl: '', ativo: true });
    const [itemImage, setItemImage] = useState(null); // Novo estado para o arquivo de imagem
    const [imagePreview, setImagePreview] = useState(''); // Novo estado para pré-visualização da imagem
    ...
    const openItemForm = (item = null) => {
        setEditingItem(item);
        // Ao abrir o formulário, preenche com os dados do item ou limpa
        setFormData(item ? { ...item } : { nome: '', descricao: '', preco: '', categoria: '', imageUrl: '', ativo: true });
        // Limpa o arquivo de imagem e a pré-visualização ao abrir
        setItemImage(null);
        // Define a pré-visualização para a URL existente se for um item em edição
        setImagePreview(item && item.imageUrl ? item.imageUrl : '');
    ...
    const closeItemForm = () => {
        ...
        setEditingItem(null);
        // Limpa os estados de imagem ao fechar o formulário
        setItemImage(null);
        setImagePreview('');
    };
    ...
    const handleFormChange = (e) => {
        ...
        if (type === 'file') {
            ...
            setItemImage(file);
            if (file) {
                // Cria uma URL de objeto para pré-visualização da imagem selecionada
                setImagePreview(URL.createObjectURL(file));
            } else {
                setImagePreview(editingItem && editingItem.imageUrl ? editingItem.imageUrl : '');
            }
        } else {
    ...
    const handleSaveItem = async (e) => {
        ...
        let finalImageUrl = formData.imageUrl; // Inicia com a URL existente do formData

        try {
            if (itemImage) { // Se um novo arquivo de imagem foi selecionado
                // Gera um nome único para a imagem para evitar colisões
                const imageName = `${establishmentId}_${Date.now()}_${itemImage.name}`;
                ...
                // Se estiver editando e havia uma imagem antiga, tente deletá-la
                if (editingItem && editingItem.imageUrl && editingItem.imageUrl !== finalImageUrl) {
                    // await deleteFileByUrl(editingItem.imageUrl); // // Descomente para ativar a exclusão da imagem antiga
                    // console.log("Imagem antiga deletada (se aplicável):", editingItem.imageUrl);
                }
            } else if (editingItem && !formData.imageUrl && editingItem.imageUrl) {
                 // Se estava editando, não selecionou nova imagem e a URL no form foi limpa (ou o campo removido),
                 // significa que o usuário quer remover a imagem existente.
                // await deleteFileByUrl(editingItem.imageUrl); // // Descomente para ativar a exclusão da imagem antiga
                 finalImageUrl = ''; // Limpa a URL no Firestore
            }
            ...
            console.error("Erro ao salvar o item com imagem:", error);
    ...
    const handleDeleteItem = async (itemId, itemName, itemImageUrl) => { // Adicione itemImageUrl
        if (window.confirm(`Tem certeza que deseja excluir o item "${itemName}"?`)) {
            try {
                // Opcional: Deletar a imagem do Storage ao deletar o item
                if (itemImageUrl) {
                   // await deleteFileByUrl(itemImageUrl); // // Descomente para ativar a exclusão da imagem antiga
                   // console.log("Imagem associada deletada (se aplicável):", itemImageUrl);
                }
    ...
    {filteredItems.map(item => (
        <tr key={item.id}>
            ...
            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-4">
                ...
                <button onClick={() => handleDeleteItem(item.id, item.nome, item.imageUrl)} className="text-red-600 hover:text-red-900">Excluir</button>
            </td>
        </tr>
    ))}
</tbody>
                        ...
                        <div className="bg-white rounded-lg shadow-xl p-6 max-w-lg w-full">
                            ...
                            <form onSubmit={handleSaveItem} className="space-y-4">
                                ...
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="itemImageUpload" className="block text-sm font-medium text-slate-700">
                                    ...
                                    {(imagePreview || formData.imageUrl) && (
                                     ...
                                     {/* Campo URL da imagem original (pode ser usado para exibir a URL atual ou para casos específicos) */}
                                    <input
                                        name="imageUrl"
                                        value={formData.imageUrl}
                                        onChange={handleFormChange}
                                        placeholder="URL da Imagem (Preenchido automaticamente)"
                                        className="w-full border-slate-300 rounded-lg"
                                        readOnly // Torna o campo somente leitura já que o upload gerencia a URL
                                        hidden // Esconda este campo se a intenção é que o usuário só use o upload
                                    />
                                </div>
