...
import {
    ...
    IoAlertCircle, IoCube, IoCash, IoPricetag, IoList, IoEyeOff, IoGrid, IoMenu,
    IoRefresh, IoBarcodeOutline
...
const ProductGridCard = ({ produto, onEdit, onDelete, onToggleStatus, stockStatus, profitMargin }) => {
  const stockConfig = {
  ...
  const mostrarPrecosVariacoes = () => {
    if (!produto.variacoes || produto.variacoes.length === 0) {
      return <p className="text-2xl font-bold text-gray-800">R$ {(Number(produto.preco) || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>;
    }
...
const StatsCard = ({ title, value, icon: Icon, colorClass, bgClass }) => (
    <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm flex items-center justify-between hover:translate-y-[-2px] transition-transform duration-300">
      ...
      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${bgClass}`}><Icon className={`text-xl ${colorClass}`} /></div>
    </div>
);
...
function AdminMenuManagement() {
  ...
  const [formData, setFormData] = useState({
    nome: '', descricao: '', categoria: '', codigoBarras: '', imageUrl: '', ativo: true, fiscal: { ncm: '', cfop: '5102', unidade: 'UN' }
  });
  ...
  const adicionarVariacao = () => {
    const novaVariacao = { id: `var-${Date.now()}-${Math.floor(Math.random() * 1000)}`, nome: '', preco: '', descricao: '', ativo: true, estoque: 0, estoqueMinimo: 0, custo: 0 };
  ...
  const atualizarVariacao = (id, field, value) => {
    ...
    setVariacoes(variacoes.map(v => v.id === id ? { ...v, [field]: finalValue } : v));
    if (variacoesErrors[id] && variacoesErrors[id][field]) {
      setVariacoesErrors(prev => ({ ...prev, [id]: { ...prev[id], [field]: undefined } }));
    }
  };

  const removerVariacao = (id) => {
    if (variacoes.length <= 1) { toast.error('O item deve ter pelo menos uma variação.'); return; }
  ...
  useEffect(() => {
    if (primeiroEstabelecimento) {
      getDoc(doc(db, 'estabelecimentos', primeiroEstabelecimento)).then(estabDoc => {
          if (estabDoc.exists()) setEstablishmentName(estabDoc.data().nome);
      }).catch(console.error);
    }
  ...
  useEffect(() => {
    ...
    const unsubscribeCategorias = onSnapshot(query(categoriasRef, orderBy('ordem', 'asc')), (categoriasSnapshot) => {
      ...
      fetchedCategories.forEach(catData => {
        ...
        const unsubscribeItems = onSnapshot(query(itemsRef), (itemsSnapshot) => {
          const novosItensDaCategoria = itemsSnapshot.docs.map(itemDoc => ({
            ...itemDoc.data(), id: itemDoc.id, categoria: catData.nome, categoriaId: catData.id, variacoes: Array.isArray(itemDoc.data().variacoes) ? itemDoc.data().variacoes : []
          }));
  ...
  const calculateProfitMargin = (precoVenda, custo) => {
    precoVenda = Number(precoVenda) || 0; custo = Number(custo) || 0;
    if (custo <= 0 || precoVenda <= 0) return 0;
    return ((precoVenda - custo) / precoVenda) * 100;
  };

  const availableCategories = useMemo(() => ['Todos', ...new Set(categories.map(cat => cat.nome).filter(Boolean))], [categories]);

  const filteredAndSortedItems = useMemo(() => {
    let filtered = menuItems.filter(item =>
      (selectedCategory === 'Todos' || item.categoria === selectedCategory) &&
      (item.nome || '').toLowerCase().includes(searchTerm.toLowerCase()) &&
      (stockFilter === 'todos' || (stockFilter === 'critico' && getStockStatus(item) === 'critico') || (stockFilter === 'baixo' && getStockStatus(item) === 'baixo') || (stockFilter === 'esgotado' && getStockStatus(item) === 'esgotado') || (stockFilter === 'normal' && getStockStatus(item) === 'normal'))
    );
  ...
  const stockStatistics = useMemo(() => {
    ...
    const totalInventoryValue = menuItems.reduce((total, item) => {
        const variationsCost = item.variacoes?.reduce((sum, v) => sum + ((Number(v.estoque) || 0) * (Number(v.custo) || 0)), 0) || 0;
  ...
  const validateForm = () => {
    ...
    if (!variacoes || variacoes.length === 0) {
      ...
      variacoes.forEach(v => {
        ...
        if (!v.preco || v.preco === '' || isNaN(precoNum)) { vError.preco = 'Obrigatório'; } else if (precoNum <= 0) { vError.preco = '> 0'; } else { temPrecoValido = true; v.preco = precoNum; }
    ...
    setFormErrors(errors); setVariacoesErrors(varErrors);
  ...
  const handleSaveItem = async (e) => {
    e.preventDefault();
    setFormLoading(true);
    if (!validateForm()) { setFormLoading(false); toast.error("Verifique os erros no formulário"); return; }
    ...
    try {
      ...
      if (itemImage) {
        ...
        if (editingItem && editingItem.imageUrl && editingItem.imageUrl !== imageUrl) {
            try { await deleteFileByUrl(editingItem.imageUrl); } catch (err) { console.warn("Erro a apagar imagem:", err); }
        }
      }

      const categoriaDoc = categories.find(cat => cat.nome === formData.categoria);
      let categoriaIdParaSalvar = editingItem ? (categoriaDoc && categoriaDoc.id !== editingItem.categoriaId ? categoriaDoc.id : editingItem.categoriaId) : (categoriaDoc ? categoriaDoc.id : null);

      if (!categoriaIdParaSalvar) {
          const novaCategoriaRef = await addDoc(collection(db, 'estabelecimentos', primeiroEstabelecimento, 'cardapio'), { nome: formData.categoria.trim().toUpperCase(), ordem: 999, ativo: true });
          categoriaIdParaSalvar = novaCategoriaRef.id;
          toast.info(`Categoria "${formData.categoria}" criada!`);
      }

      const precosAtivos = variacoes.filter(v => v.ativo !== false && Number(v.preco) > 0).map(v => Number(v.preco));
      const precoPrincipal = precosAtivos.length > 0 ? Math.min(...precosAtivos) : 0;
      const variationsToSave = variacoes.map(v => ({ ...v, preco: Number(v.preco), estoque: Number(v.estoque) || 0, estoqueMinimo: Number(v.estoqueMinimo) || 0, custo: Number(v.custo) || 0 }));
      ...
      const itemData = {
        nome: formData.nome.trim(), descricao: formData.descricao?.trim() || '', codigoBarras: formData.codigoBarras?.trim() || '',
        preco: precoPrincipal, variacoes: variationsToSave, categoria: formData.categoria.trim().toUpperCase(),
        imageUrl: imageUrl, ativo: formData.ativo, estoque: totalStock, estoqueMinimo: totalEstoqueMinimo, custo: custoPrincipal, 
        fiscal: formData.fiscal, atualizadoEm: new Date()
      };

      if (editingItem) {
        if (editingItem.categoriaId !== categoriaIdParaSalvar) {
            await setDoc(doc(db, 'estabelecimentos', primeiroEstabelecimento, 'cardapio', categoriaIdParaSalvar, 'itens', editingItem.id), { ...itemData, criadoEm: editingItem.criadoEm || new Date() });
      ...
      closeItemForm();
    } catch (error) { console.error("Erro ao salvar:", error); toast.error("Erro ao salvar item."); } finally { setFormLoading(false); }
  };
  ...
  const toggleItemStatus = async (item) => {
    try { await updateDoc(doc(db, 'estabelecimentos', primeiroEstabelecimento, 'cardapio', item.categoriaId, 'itens', item.id), { ativo: item.ativo === false ? true : false, atualizadoEm: new Date() }); } catch(e) { toast.error("Erro ao alterar status"); }
  };
  ...
  const activateAllItems = async () => {
    ...
    try {
        ...
        inativos.forEach(item => { const ref = doc(db, 'estabelecimentos', primeiroEstabelecimento, 'cardapio', item.categoriaId, 'itens', item.id); batch.update(ref, { ativo: true, atualizadoEm: new Date() }); });
        await batch.commit(); toast.success(`${inativos.length} itens ativados!`); setShowActivateAllModal(false);
  ...
  const openItemForm = useCallback((item = null) => {
    if (item) {
      setEditingItem(item);
      setFormData({ nome: item.nome || '', descricao: item.descricao || '', categoria: item.categoria || '', codigoBarras: item.codigoBarras || '', imageUrl: item.imageUrl || '', ativo: item.ativo !== false, fiscal: item.fiscal || { ncm: '', cfop: '5102', unidade: 'UN' } });
      setTermoNcm(item.fiscal?.ncm || '');
      const loadedVariacoes = item.variacoes && item.variacoes.length > 0 
        ? item.variacoes.map((v, index) => ({ ...v, id: v.id || `var-${Date.now()}-${index}`, nome: v.nome || 'Padrão', preco: parsePrecoSeguro(v.preco).toString(), estoque: Number(v.estoque) || 0, estoqueMinimo: Number(v.estoqueMinimo) || 0, custo: Number(v.custo || item.custo) || 0, ativo: v.ativo !== false })) 
        : [{ id: `var-${Date.now()}-default`, nome: 'Padrão', preco: parsePrecoSeguro(item.preco).toString(), ativo: true, estoque: Number(item.estoque) || 0, estoqueMinimo: Number(item.estoqueMinimo) || 0, custo: Number(item.custo) || 0 }];
    ...
    setFormErrors({}); setShowItemForm(true);
  }, []);

  const closeItemForm = () => {
    setShowItemForm(false); setEditingItem(null); setFormData({ nome: '', descricao: '', categoria: '', codigoBarras: '', imageUrl: '', ativo: true, fiscal: { ncm: '', cfop: '5102', unidade: 'UN' } });
    setTermoNcm(''); setNcmResultados([]); setVariacoes([]); setImagePreview(''); setItemImage(null);
  };

  const handleFormChange = (e) => {
    const { name, value, type, checked, files } = e.target;
    if (type === 'file') { if (files[0]) { setItemImage(files[0]); setImagePreview(URL.createObjectURL(files[0])); } } else if (type === 'checkbox') { setFormData(prev => ({ ...prev, [name]: checked })); } else { setFormData(prev => ({ ...prev, [name]: value })); }
  };

  const handleFiscalChange = (e) => {
    const { name, value } = e.target;
    setFormData(prev => ({ ...prev, fiscal: { ...(prev.fiscal || { ncm: '', cfop: '5102', unidade: 'UN' }), [name]: value } }));
  };

  const buscarNcm = async (termo) => {
    try {
      const texto = termo || ''; setTermoNcm(texto); handleFiscalChange({ target: { name: 'ncm', value: texto } });
      if (texto.length < 3) { setNcmResultados([]); return; }
      setPesquisandoNcm(true);
      const response = await fetch(`https://brasilapi.com.br/api/ncm/v1?search=${texto}`);
      if (response.ok) { const data = await response.json(); setNcmResultados(Array.isArray(data) ? data.slice(0, 15) : []); } else { setNcmResultados([]); }
    } catch (error) { setNcmResultados([]); } finally { setPesquisandoNcm(false); }
  };

  const selecionarNcm = (codigo) => { handleFiscalChange({ target: { name: 'ncm', value: codigo } }); setTermoNcm(codigo); setNcmResultados([]); };
    ...
    const actions = (
        <div className="flex items-center space-x-2 md:space-x-3">
            ...
            {stockStatistics.inactiveItems > 0 && (
                <button onClick={() => setShowActivateAllModal(true)} className="flex items-center gap-2 bg-white text-gray-600 hover:text-green-600 border border-gray-200 hover:border-green-200 font-bold py-2 px-4 rounded-xl transition-all shadow-sm text-sm"><IoRefresh /> <span className="hidden sm:inline">Ativar Todos ({stockStatistics.inactiveItems})</span></button>
            )}
            <button onClick={() => openItemForm()} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-xl shadow-lg shadow-blue-200 text-sm transition-all transform hover:scale-105"><IoAddCircleOutline className="text-xl"/> <span className="hidden sm:inline">Novo Item</span></button>
        </div>
    );
    setActions(actions); return () => clearActions();
  }, [viewMode, stockStatistics, setActions, clearActions, openItemForm]);

  if (!primeiroEstabelecimento) return <div className="p-8 text-center text-gray-500">Carregando...</div>;
  if (loading) return <div className="p-6 max-w-7xl mx-auto"><SkeletonLoader /></div>;

  return (
    <div className="min-h-screen bg-[#F3F4F6] p-4 md:p-6 font-sans pb-24 relative">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3 mb-8">
            ...
            <div className="col-span-2 md:col-span-2 xl:col-span-2">
                <StatsCard title="Valor em Estoque (Custo)" value={`R$ ${stockStatistics.totalInventoryValue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`} icon={IoCash} colorClass="text-indigo-600" bgClass="bg-indigo-50" />
            </div>
        </div>
        ...
        <div className="sticky top-2 z-30 bg-white/90 backdrop-blur-md rounded-2xl shadow-sm border border-gray-200 p-2 mb-8 flex flex-col md:flex-row gap-2 md:items-center">
            <div className="relative flex-1">
                ...
                <input type="text" placeholder="Buscar produto..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border-transparent focus:bg-white focus:ring-2 focus:ring-blue-100 rounded-xl transition-all outline-none" />
            </div>
            <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
                <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="px-4 py-2.5 bg-gray-50 hover:bg-white border border-transparent hover:border-gray-200 rounded-xl cursor-pointer outline-none transition-all text-sm font-medium text-gray-700 min-w-[150px]">
                    {availableCategories.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
                <select value={stockFilter} onChange={(e) => setStockFilter(e.target.value)} className="px-4 py-2.5 bg-gray-50 hover:bg-white border border-transparent hover:border-gray-200 rounded-xl cursor-pointer outline-none transition-all text-sm font-medium text-gray-700 min-w-[150px]">
            ...
            {paginatedItems.length > 0 ? (
                <div className={viewMode === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" : "space-y-3"}>
                    {paginatedItems.map(item => (
                        viewMode === 'grid' ? (
                            <ProductGridCard key={item.id} produto={item} onEdit={() => openItemForm(item)} onDelete={() => handleDeleteItem(item)} onToggleStatus={() => toggleItemStatus(item)} stockStatus={getStockStatus(item)} profitMargin={calculateProfitMargin(item.preco, item.custo)} />
                        ) : (
                            <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between hover:border-blue-200 transition-colors">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-2 text-xs text-gray-500"><span>{item.categoria}</span><span>•</span><span className={item.ativo !== false ? 'text-green-600' : 'text-red-500'}>{item.ativo !== false ? 'Ativo' : 'Inativo'}</span></div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                                    <div className="text-right hidden sm:block"><p className="font-bold">R$ {Number(item.preco).toFixed(2)}</p><p className="text-xs text-gray-400">Estoque: {item.estoque}</p></div>
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-gray-300">
                    <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mb-4"><IoCube className="text-4xl text-gray-300" /></div>
                    ...
                    <button onClick={() => openItemForm()} className="text-blue-600 font-bold hover:underline">+ Adicionar Novo Item</button>
                </div>
            )}
        </div>
        ...
        {/* 👇 MODAL DE CADASTRO/EDIÇÃO (TELA CHEIA FORÇADA NO INLINE CSS PARA NUNCA BUGAR) 👇 */}
        {showItemForm && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 999999, backgroundColor: '#f9fafb', display: 'flex', flexDirection: 'column' }}>
            
            {/* HEADER FIXO ABSOLUTO */}
            <div style={{ width: '100%', height: '80px', backgroundColor: '#fff', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', flexShrink: 0, zIndex: 10 }}>
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800">{editingItem ? 'Editar Produto' : 'Novo Produto'}</h2>
                <p className="text-xs sm:text-sm text-gray-500">Gerenciamento completo do item</p>
              </div>
              <button onClick={closeItemForm} className="px-5 py-2 rounded-xl bg-gray-100 hover:bg-red-50 text-gray-600 hover:text-red-500 font-bold transition-all flex items-center gap-2">
                <span className="hidden sm:block">Fechar</span>
                <IoClose size={24} />
              </button>
            </div>

            {/* CORPO ROLÁVEL COM FORMULÁRIO COMPLETO */}
            <form onSubmit={handleSaveItem} style={{ flex: 1, overflowY: 'auto', position: 'relative', display: 'flex', flexDirection: 'column' }} className="custom-scrollbar">
              
              <div className="w-full max-w-6xl mx-auto space-y-8 p-4 sm:p-8" style={{ paddingBottom: '160px' }}>
                ...
                <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input type="text" name="nome" value={formData.nome} onChange={handleFormChange} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Ex: X-Burger Especial" required />
                      {formErrors.nome && <span className="text-red-500 text-xs mt-1 block">{formErrors.nome}</span>}
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-semibold mb-2">Categoria *</label>
                        <input type="text" name="categoria" value={formData.categoria} onChange={handleFormChange} list="cat-list" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Digite ou selecione" required disabled={!!editingItem} />
                        <datalist id="cat-list">{categories.map(c => (<option key={c.id} value={c.nome} />))}</datalist>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold mb-2 flex items-center gap-1"><IoBarcodeOutline className="text-lg text-gray-500"/> Cód Barras</label>
                        <input type="text" name="codigoBarras" value={formData.codigoBarras} onChange={handleFormChange} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-mono" placeholder="EAN/GTIN" />
                      </div>
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-2">Descrição</label>
                    <textarea name="descricao" value={formData.descricao} onChange={handleFormChange} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none resize-none" rows={3} placeholder="Ingredientes, detalhes..." />
                  </div>
                </div>

                {/* BLOCO 2: VARIAÇÕES E ESTOQUE */}
                <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-100 pb-4 gap-4">
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-xl w-full sm:w-auto">
                      <button type="button" onClick={() => setVariacoes([{id: `var-${Date.now()}-unico`, nome: 'Padrão', preco: variacoes[0]?.preco || '', ativo: true, estoque: variacoes[0]?.estoque || 0, estoqueMinimo: variacoes[0]?.estoqueMinimo || 0, custo: variacoes[0]?.custo || 0 }])} className={`flex-1 sm:flex-none px-5 py-3 rounded-lg text-sm font-bold transition-all ${variacoes.length === 1 && variacoes[0].nome === 'Padrão' ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Preço Único</button>
                      <button type="button" onClick={() => { if(variacoes.length===1 && variacoes[0].nome==='Padrão') setVariacoes([{id: `var-${Date.now()}-multi`, nome: 'Tamanho Único', preco: variacoes[0].preco, ativo: true, estoque: variacoes[0].estoque, estoqueMinimo: variacoes[0].estoqueMinimo, custo: variacoes[0].custo}]); }} className={`flex-1 sm:flex-none px-5 py-3 rounded-lg text-sm font-bold transition-all ${variacoes.length > 1 || variacoes[0].nome !== 'Padrão' ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Múltiplos Tamanhos</button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    {variacoes.map((v) => (
                      <div key={v.id} className="bg-gray-50 p-5 rounded-2xl border border-gray-200 relative group">
                        {variacoes.length > 1 && <button type="button" onClick={() => removerVariacao(v.id)} className="absolute -top-3 -right-3 bg-red-100 text-red-600 p-2 rounded-full shadow-md hover:bg-red-200 transition-colors z-10"><IoClose size={18}/></button>}
                ...
                {/* BLOCO 3: FISCAL E IMAGEM */}
                <div className="grid md:grid-cols-2 gap-6">
                    <div className="bg-emerald-50 p-6 sm:p-8 rounded-3xl border border-emerald-100 shadow-sm space-y-6">
                        <h3 className="text-lg font-bold text-emerald-800 flex items-center gap-2">🏢 Dados NFC-e</h3>
                        <div className="space-y-4">
                            <div className="relative">
                                <label className="block text-sm font-semibold text-emerald-800 mb-2">NCM (Busca BrasilAPI)</label>
                                <input type="text" name="ncm" value={termoNcm || formData.fiscal?.ncm || ''} onChange={(e) => buscarNcm(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') e.preventDefault(); }} className="w-full p-4 bg-white border border-emerald-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="Ex: Hambúrguer, 2202..." autoComplete="off" />
                                ...
                                {ncmResultados.length > 0 && (
                                    <div className="absolute z-50 w-full mt-2 bg-white border border-emerald-200 rounded-2xl shadow-2xl max-h-60 overflow-y-auto">
                                        {ncmResultados.map((item) => (
                                            <div key={item.codigo} onClick={() => selecionarNcm(item.codigo)} className="p-4 border-b border-gray-100 hover:bg-emerald-50 cursor-pointer transition-colors">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="cfop" value={formData.fiscal?.cfop || ''} onChange={handleFiscalChange} className="w-full p-4 bg-white border border-emerald-200 rounded-2xl outline-none">
                                        <option value="5102">5102 - Normal</option>
                                        <option value="5405">5405 - ST</option>
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-emerald-800 mb-2">Medida</label>
                                    <select name="unidade" value={formData.fiscal?.unidade || 'UN'} onChange={handleFiscalChange} className="w-full p-4 bg-white border border-emerald-200 rounded-2xl outline-none">
                    ...
                    <div className="space-y-6 flex flex-col justify-between">
                        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-sm flex flex-col items-center justify-center gap-4 flex-1">
                                <div className="w-32 h-32 bg-gray-50 rounded-2xl border border-gray-200 flex items-center justify-center overflow-hidden shrink-0">
                                {imagePreview ? <img src={imagePreview} className="w-full h-full object-cover" /> : <IoImageOutline className="text-5xl text-gray-300"/>}
                                </div>
                                <div className="w-full text-center">
                                    <label className="block text-base font-bold text-gray-800 mb-3">Foto do Produto</label>
              ...
              <div style={{ position: 'fixed', bottom: 0, left: 0, width: '100vw', backgroundColor: '#fff', borderTop: '1px solid #e5e7eb', padding: '16px 24px', display: 'flex', justifyContent: 'flex-end', gap: '16px', zIndex: 20, boxShadow: '0 -10px 40px rgba(0,0,0,0.1)' }}>
                  <button type="button" onClick={closeItemForm} className="px-8 py-4 rounded-xl border border-gray-300 font-semibold text-gray-600 hover:bg-gray-100 transition-all text-lg hidden sm:block">
                      Cancelar
                  </button>
                  <button type="submit" disabled={formLoading} className="w-full sm:w-auto px-12 py-4 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-700 shadow-xl shadow-blue-200 transition-all text-lg flex items-center justify-center gap-2">
