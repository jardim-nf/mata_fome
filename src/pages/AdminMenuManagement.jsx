// src/pages/AdminMenuManagement.jsx - VERSÃO COMPLETA E FINAL
...
function AdminMenuManagement() {
  ...
  const openItemForm = useCallback((item = null) => {
    if (item) {
      ...
      setVariacoes(item.variacoes?.length ? item.variacoes.map(v => ({...v, id: v.id || Date.now().toString()})) : [{ id: Date.now().toString(), nome: 'Padrão', preco: item.preco || '', ativo: true }]);
      setImagePreview(item.imageUrl || '');
    } else {
      setEditingItem(null);
      setFormData({ nome: '', descricao: '', categoria: '', imageUrl: '', ativo: true, estoque: 0, estoqueMinimo: 0, custo: 0 });
      setVariacoes([{ id: Date.now().toString(), nome: 'Padrão', preco: '', descricao: '', ativo: true }]);
      setImagePreview('');
    }
    setFormErrors({});
                ...
                <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                    ...
                    <form onSubmit={handleSaveItem} className="p-6 space-y-6">
                        ...
                        <div className="bg-gray-50 p-6 rounded-2xl border">
                            <h3 className="font-bold mb-4 flex items-center"><IoCash className="mr-2"/> Preços</h3>
                            
                            <div className="flex gap-4 mb-4">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" checked={variacoes.length === 1 && variacoes[0].nome === 'Padrão'} onChange={() => setVariacoes([{id: Date.now().toString(), nome: 'Padrão', preco: variacoes[0]?.preco || '', ativo: true}])} />
                                    Produto Simples
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" checked={variacoes.length > 1 || (variacoes.length === 1 && variacoes[0].nome !== 'Padrão')} onChange={() => { if(variacoes.length===1 && variacoes[0].nome==='Padrão') setVariacoes([{id: Date.now().toString(), nome: 'Tamanho Único', preco: variacoes[0].preco, ativo: true}]); }} />
                                    Com Variações
                                </label>
                            </div>
                            ...
                            {(variacoes.length > 1 || (variacoes.length === 1 && variacoes[0].nome !== 'Padrão')) && (
                                <button type="button" onClick={adicionarVariacao} className="text-blue-600 font-bold text-sm mb-4 flex items-center gap-1"><IoAddCircleOutline/> Adicionar Variação</button>
                            )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-blue-50 p-6 rounded-2xl border border-blue-200">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div><label className="text-xs">Mínimo</label><input type="number" name="estoqueMinimo" value={formData.estoqueMinimo} onChange={handleFormChange} className="w-full p-2 border rounded-lg" /></div>
