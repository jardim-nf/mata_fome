...
function ComandaView() {
  ...
  // REMOVEMOS O useEffect QUE CHAMAVA window.print() AUTOMATICAMENTE

  // Função para imprimir a comanda manualmente
  const handlePrint = () => {
    window.print();
  };

  // Função para voltar ao painel de pedidos
  const handleBackToPainel = () => {
    navigate('/painel'); // Navega de volta para a rota do painel
  };
  ...
  return (
    <div className="bg-[var(--bege-claro)] min-h-screen p-4 flex flex-col items-center">
      ...
      <div className="w-full max-w-sm mb-4 flex justify-between gap-2 no-print"> {/* Adicionada classe 'no-print' */}
        <button
