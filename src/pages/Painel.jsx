...
function Painel() {
    ...
    const STATUS_UI = useMemo(() => {
        ...
        return {
            ...
            preparo: { title: tipo === 'varejo' ? 'Em Separação' : 'Em Preparo', icon: tipo === 'varejo' ? '📦' : '🔥', dot: 'bg-amber-500', bgBadge: 'bg-amber-100', textBadge: 'text-amber-700', emptyTitle: tipo === 'varejo' ? 'Estoque livre' : `${termCozinha} livre`, emptyMsg: tipo === 'varejo' ? 'Nenhum pedido em separação' : 'Nenhum pedido em preparo' },
            em_entrega: { title: tipo === 'varejo' ? 'Em Envio / Rota' : 'Em Entrega', icon: tipo === 'varejo' ? '🚚' : '🛵', dot: 'bg-blue-500', bgBadge: 'bg-blue-100', textBadge: 'text-blue-700', emptyTitle: tipo === 'varejo' ? 'Sem envios' : 'Sem entregas', emptyMsg: tipo === 'varejo' ? 'Nenhum pedido em rota de envio' : 'Nenhum pedido na rua' },
            pronto_para_servir: { title: abaAtiva === 'cozinha' ? `Pronto (${termMesa})` : (tipo === 'varejo' ? 'Pronto p/ Retirada' : 'Pronto p/ Retirada'), icon: '✅', dot: 'bg-emerald-500', bgBadge: 'bg-emerald-100', textBadge: 'text-emerald-700', emptyTitle: 'Nada pronto', emptyMsg: abaAtiva === 'cozinha' ? 'Aparecerão aqui quando ficarem prontos' : (tipo === 'varejo' ? 'Pedidos prontos para retirada aparecerão aqui' : 'Pedidos de retirada prontos aparecerão aqui') },
        ...
        <div className="bg-white border-b border-gray-200">
            <div className="flex flex-col md:flex-row justify-between items-center px-4 py-2 gap-4">
                <div className="flex items-center gap-3">
                    <BackButton to="/admin" />
                    <div className="hidden md:flex flex-col"><span className="text-sm font-semibold text-gray-800">{estabelecimentoInfo?.nome || userData?.nomeEstabelecimento || "Painel de Pedidos"}</span><span className="text-xs text-gray-500">{estabelecimentoInfo?.tipoNegocio === 'varejo' ? 'Painel de Separação & Envio' : 'KDS (Painel de Pedidos)'}</span></div>
