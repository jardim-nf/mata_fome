...
const CalculadoraTab = ({
  ...
  dbVidros, dbCores, dbKits, dbModelos, pedidos,
  alertaSeguranca,
  handleCreateOS,
  handleCepLookup,
  aplicarFolgasPadrao,
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 text-left">
      ...
      <div className="glass-card lg:col-span-7 p-4 sm:p-6 space-y-4 sm:space-y-5">
        ...
        <div className="flex flex-wrap gap-1.5 pb-3 border-b border-slate-200">
          <span className="text-[8px] font-black uppercase text-slate-400 tracking-wider w-full mb-1">⚡ Templates Rápidos</span>
          {[
            { label: 'Box Suite', emoji: '🛀', m: 'box', mn: 'Blindex Standard', l: 1400, h: 1900, alum: 'fosco', pux: 'padrao' },
            { label: 'Box Quadrado', emoji: '🚿', m: 'box', mn: 'Elegance', l: 900, h: 1900, alum: 'preto', pux: 'padrao' },
            { label: 'Janela 4F', emoji: '🪟', m: 'janela', mn: 'Janela Correr 4F', l: 1600, h: 1200, alum: 'branco', pux: 'sem' },
            { label: 'Porta Abrir', emoji: '🚪', m: 'porta', mn: 'Porta Pivotante', l: 900, h: 2100, alum: 'preto', pux: 'padrao' },
            { label: 'Espelho', emoji: '🪞', m: 'espelho', mn: 'Espelho Banheiro', l: 800, h: 1000, alum: 'fosco', pux: 'sem' }
          ].map(tpl => (
            <button
              key={tpl.label}
              type="button"
              onClick={() => {
                setModelo(tpl.m);
                setModeloNomeCompleto(tpl.mn);
                setLargura(tpl.l);
                setAltura(tpl.h);
                setCorAluminio(tpl.alum);
                setTipoPuxador(tpl.pux);
                toast.success(`⚡ Template "${tpl.label}" aplicado!`);
              }}
              className="px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[9px] font-bold text-slate-600 hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all flex items-center gap-1"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {/* Spacer on desktop to align layout */}
          <div className="hidden sm:block"></div>

          {/* 2. Tamanho (Largura e Altura) */}
          <div>
          ...
          {/* 3. Banners de Alerta de Segurança (NBR 7199 - Colapsável) */}
          {alertaSeguranca && (() => {
            const isSuccess = alertaSeguranca.tipo === 'success';
            const expanded = showCalcSafetyDetails !== null ? showCalcSafetyDetails : !isSuccess;
            
            return (
              <div className={`sm:col-span-2 rounded-xl border flex flex-col shadow-sm overflow-hidden transition-all duration-300 ${
                ...
                {expanded && (
                  <div className="p-3.5 border-t border-slate-200/50 bg-white/50 space-y-3">
                    ...
                    {alertaSeguranca.checklist && alertaSeguranca.checklist.length > 0 && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 pt-3 border-t border-slate-200/60">
          ...
          {/* 4. Tipo de Vidro e Cor do Vidro */}
          <div>
          ...
          <div className="sm:col-span-2 text-left">
            ...
            {showPerfilSelector && (() => {
              const filteredPerfis = PERFIS_ALUMINIO.filter(p => {
                if (!searchPerfilQuery) return true;
                const q = searchPerfilQuery.toLowerCase();
                return p.codigo.toLowerCase().includes(q) || p.nome.toLowerCase().includes(q) || p.categoria.toLowerCase().includes(q);
              });
              return (
          ...
          <div>
            ...
            <select value={corAluminio} onChange={e => setCorAluminio(e.target.value)} className="glass-input w-full">
              <option value="fosco">Fosco Natural (Anodizado)</option>
              <option value="branco">Branco (Pintura Epóxi)</option>
              <option value="preto">Preto Fosco</option>
              <option value="preto_anod">Preto Brilhante (Anodizado)</option>
              <option value="bronze">Bronze Escuro</option>
              <option value="brilhante">Cromado / Polido / Brilhante</option>
              <option value="inox">Aço Inox Escovado</option>
              <option value="dourado">Dourado / Gold Premium</option>
              <option value="rose">Champagne / Rosé Gold</option>
              <option value="grafite">Cinza Grafite / Antracite</option>
              <option value="corten">Aço Corten / Rustico</option>
              <option value="amadeirado">Textura Amadeirada</option>
            </select>
          </div>
          ...
          <div>
            ...
            <select value={tipoPuxador} onChange={e => setTipoPuxador(e.target.value)} className="glass-input w-full">
              <option value="padrao">Puxador H Standard (30cm)</option>
              <option value="h_simples">Puxador H Simples (40cm)</option>
              <option value="barra">Puxador de Barra Tubular (60cm)</option>
              <option value="barra_45">Puxador Angular 45º (80cm)</option>
              <option value="knob">Puxador Redondo / Knob (Botão)</option>
              <option value="concha">Puxador Tipo Concha (Sobrepor)</option>
              <option value="furo">Apenas Furo (Puxador Embutido)</option>
          ...
          <div>
            ...
            <select value={cenarioFundo} onChange={e => setCenarioFundo(e.target.value)} className="glass-input w-full">
              <option value="banheiro_premium">🛁 Banheiro Premium (Claro)</option>
              <option value="banheiro_rustico">🪵 Banheiro Rústico (Madeira/Escuro)</option>
              <option value="sala_tijolo">🧱 Sala Industrial (Tijolo)</option>
              <option value="escritorio_concreto">🏢 Escritório (Concreto)</option>
            </select>
          </div>
          ...
          <div>
            ...
            <select value={anguloCorte} onChange={e => setAnguloCorte(e.target.value)} className="glass-input w-full font-bold">
              <option value="90">90º (Corte Reto)</option>
              <option value="45">45º (Meia Esquadria)</option>
            </select>
          </div>
          ...
          <div>
            <label className="text-[10px] font-extrabold uppercase text-slate-500 block mb-1">Desconto Altura (mm)</label>
            <input type="number" placeholder="Ex: 50" value={descontoAltura} onChange={e => setDescontoAltura(parseInt(e.target.value) || 0)} className="glass-input w-full font-bold" />
          </div>
      ...
      <div className="lg:col-span-5 flex flex-col gap-6">
        ...
        <div className="glass-card p-5 flex flex-col items-center justify-center min-h-[300px] relative overflow-hidden group">
          ...
          <div className="text-[10px] text-slate-500 font-extrabold mt-1 text-center bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-lg w-full">
