...
  useEffect(() => {
 ...
   if (!currentUser || !isMasterAdmin) {
  ...
  return;
   }
   setLoadingUsers(false); // Definido aqui após a verificação de permissão
 }
...
  const fetchUsuarios = useCallback(async (direction = 'next', startDoc = null, resetPagination = false) => {
 if (!isMasterAdmin || !currentUser) { // Verifica permissão antes de buscar
 ...
 // 'nome' é um bom campo se todos os usuários o tiverem.
 // Se 'nome' puder estar vazio ou nulo, considere ordenar por 'email' ou 'uid'.
 ...
   // Para paginação reversa, invertemos a ordem para endBefore e depois revertemos os resultados.
   q = query(baseQueryRef, ...queryConstraints, orderBy(orderByField, 'desc'), startAfter(startDoc), limit(ITEMS_PER_PAGE)); // Usamos startAfter com orderBy('desc')
 } else { // Fallback para primeira carga
   q = query(baseQueryRef, ...queryConstraints, orderBy(orderByField, 'asc'), limit(ITEMS_PER_PAGE));
 }
 ...
   
   // Se for "anterior", precisamos reverter a ordem se a query foi invertida
   if (direction === 'prev') {
 ...
   if (documentSnapshots.docs.length > 0) {
  ...
  // Para verificar 'hasPrevious', fazemos uma query reversa a partir do primeiro item da página atual
 ...
   setError("Erro ao carregar a lista de usuários. Verifique suas permissões.");
 } finally {
   setLoadingUsers(false);
 }
  }, [isMasterAdmin, currentUser, filterRole, filterStatus]); // Removido searchTerm das dependências de fetchUsuarios
...
  useEffect(() => {
 ...
   setHasMore(true);
   setHasPrevious(false);
   fetchUsuarios('next', null, true); // True para resetPagination (primeira página)
 } else if (!authLoading && (!currentUser || !isMasterAdmin)) {
   setLoadingUsers(false); 
 }
  }, [authLoading, isMasterAdmin, currentUser, filterRole, filterStatus, searchTerm, fetchUsuarios]); // searchTerm agora é uma dependência aqui para acionar o fetch quando ele muda.
...
  // Este filtro agora opera sobre a lista 'usuarios' já carregada
...
  const toggleUserAtivo = async (userId, currentStatus, userName) => {
 console.log(`Tentando alternar status para usuário: ${userName} (ID: ${userId}) de ${currentStatus} para ${!currentStatus}`);
 ...
   console.log("Status do usuário atualizado no Firestore. Disparando auditLogger...");
   auditLogger(
 ...
   // Após o update, re-fetch para garantir que a UI está sincronizada
   fetchUsuarios('next', null, true); // Recarrega a primeira página após o update
 } catch (error) {
   console.error("Erro ao alternar status do usuário:", error);
   if (error.code === 'permission-denied') {
  toast.error('Permissão negada. Você não pode alterar o status deste usuário.');
   } else {
...
  const handleDeleteUser = async (userId, userName) => {
 console.log(`Tentando deletar usuário: ${userName} (ID: ${userId})`);
 if (currentUser.uid === userId) {
 ...
   try {
  ...
  console.log("Documento do usuário deletado do Firestore. Disparando auditLogger...");
  auditLogger(
   'USUARIO_DELETADO',
   { uid: currentUser.uid, email: currentUser.email, role: 'masterAdmin' },
   { type: 'usuario', id: userId, name: userName }
  ...
  fetchUsuarios('next', null, true); // Recarrega a primeira página após o delete
   } catch (error) {
  console.error("Erro ao deletar usuário:", error);
  if (error.code === 'permission-denied') {
    toast.error('Permissão negada. Você não pode deletar este usuário.');
  } else {
   ...
     <table className="min-w-full divide-y divide-gray-200">
    ...
      {filteredUsuariosBySearchTerm.map((user) => (
     ...
       <td className="px-6 py-4 whitespace-nowrap text-sm">
      ...
        user.isAdmin ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-600'
      }`}>
     ...
       <td className="px-6 py-4 whitespace-nowrap text-sm">
      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
        user.isMasterAdmin ? 'bg-yellow-500 text-black' : 'bg-gray-100 text-gray-600'
      }`}>
