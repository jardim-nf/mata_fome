...
import { createBrazilFlagCanvas } from '../utils/renderHelpers';

/**
 * useThreeScene — Encapsulates the entire Three.js 3D scene setup, rendering,
 * avatar creation, animation loop, and cleanup.
 * 
 * Extracted from the monolithic SquadMeeting3D component (~2,445 lines).
 * 
 * @param {Object} params
 * @param {React.RefObject} params.viewportRef - DOM element ref for the 3D viewport
 * @param {React.RefObject} params.stateRef - Mutable state shared between React and Three.js loop
 * @param {React.RefObject} params.speechTextRef - Current speech text ref
 * @param {React.RefObject} params.themeRef - Current theme ref
 * @param {React.RefObject} params.hologramModelRef - Hologram group ref (output)
 * @param {React.RefObject} params.rendererRef - WebGL renderer ref (output)
 * @param {Object} params.bubblesRefs - Object of refs for speech bubble DOM elements
 * @param {Function} params.updateHudCanvas - HUD canvas update function
 * @param {Function} params.addLog - Log function
 * @param {Function} params.setActiveAgent - State setter for active agent
 * @param {Function} params.setIsCoffeeBreak - State setter for coffee break
 */
export function useThreeScene({
  ...
  useEffect(() => {
    ...
    // NORMAL_POSITIONS and IS_SITTING_NORMAL are now imported from constants
    const isSittingNormal = IS_SITTING_NORMAL;
    ...
    const bloomPass = new UnrealBloomPass(new THREE.Vector2(currentWidth, currentHeight), 0.6, 0.3, 0.85);
    ...
    const hemiLight = new THREE.HemisphereLight(0xffffff, 0xf1f5f9, 0.8);
    scene.add(hemiLight);

    const spotlight = new THREE.SpotLight(0xfff7ed, 1.8); // Luz direcional quente
    ...
    const accentLight = new THREE.PointLight(0x4f46e5, 1.5, 12); // Luz de destaque indigo
    ...
    // Generate Brazil Flags decoration for World Cup
    const flagCanvas = createBrazilFlagCanvas();
    ...
    // Hanging flag on the right wall (Z = -7.47, facing +Z)
    const flagWallR = new THREE.Mesh(new THREE.PlaneGeometry(2.0, 1.4), flagMat);
    flagWallR.position.set(-3.0, 3.2, -7.43);
    flagWallR.receiveShadow = true;
    flagWallR.castShadow = true;
    scene.add(flagWallR);

    // Hanging flag on the left wall (X = -7.47, facing +X)
    const flagWallL = new THREE.Mesh(new THREE.PlaneGeometry(2.0, 1.4), flagMat);
    flagWallL.position.set(-7.43, 3.2, 0.0);
    flagWallL.rotation.y = Math.PI / 2;
    flagWallL.receiveShadow = true;
    flagWallL.castShadow = true;
    scene.add(flagWallL);

    // Standing flags on poles in corners of the room
    const createStandingFlag = (x, z, rotationY) => {
      const flagGroup = new THREE.Group();
      flagGroup.position.set(x, 0, z);
      flagGroup.rotation.y = rotationY;

      // Brass Base
      const baseGeo = new THREE.CylinderGeometry(0.2, 0.2, 0.08, 12);
      const baseMat = new THREE.MeshStandardMaterial({ color: 0xcca43b, metalness: 0.8, roughness: 0.2 });
      ...
      const poleMat = new THREE.MeshStandardMaterial({ color: 0xd4d4d8, metalness: 0.9, roughness: 0.1 });
      ...
      // Flag Plane (attached to pole at X = 0.4, Y = 2.0)
      const flagGeo = new THREE.PlaneGeometry(0.8, 0.56);
      const flagMesh = new THREE.Mesh(flagGeo, flagMat);
      flagMesh.position.set(0.4, 2.0, 0);
    ...
    // Position standing flags in the corners/edges
    createStandingFlag(5.5, -6.5, -Math.PI / 4); // Behind Sheldon
    createStandingFlag(-6.5, -5.5, Math.PI / 4);  // Behind Nairobi/Coffee
    createStandingFlag(-6.5, 5.5, Math.PI * 0.75); // Left Front corner
    ...
    // MacBook Pro Materials
    const macbookSilverMat = new THREE.MeshStandardMaterial({ color: 0xd4d4d8, metalness: 0.9, roughness: 0.2 });
    const macbookKeyboardMat = new THREE.MeshStandardMaterial({ color: 0x18181b, roughness: 0.6 });
    const macbookTrackpadMat = new THREE.MeshStandardMaterial({ color: 0xa1a1aa, metalness: 0.8, roughness: 0.3 });

    const createMacbook = (displayGlowColor) => {
      const macGroup = new THREE.Group();

      // Bottom Base
      const baseMesh = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.012, 0.17), macbookSilverMat);
      baseMesh.position.y = 0.006;
      baseMesh.castShadow = true;
      baseMesh.receiveShadow = true;
      macGroup.add(baseMesh);

      // Keyboard
      const keyboardMesh = new THREE.Mesh(new THREE.BoxGeometry(0.21, 0.002, 0.09), macbookKeyboardMat);
      keyboardMesh.position.set(0, 0.013, 0.025);
      macGroup.add(keyboardMesh);

      // Trackpad
      const trackpadMesh = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.002, 0.04), macbookTrackpadMat);
      trackpadMesh.position.set(0, 0.013, -0.05);
      macGroup.add(trackpadMesh);

      // Lid & Screen (grouped to pivot at the back edge)
      const lidGroup = new THREE.Group();
      lidGroup.position.set(0, 0.01, -0.08); // Pivot point at the back edge of the base

      const lidMesh = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.16, 0.008), macbookSilverMat);
      lidMesh.position.set(0, 0.08, 0);
      lidMesh.castShadow = true;
      lidMesh.receiveShadow = true;
      lidGroup.add(lidMesh);

      // Screen Display
      const displayMesh = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.14, 0.002), new THREE.MeshBasicMaterial({ color: displayGlowColor }));
      displayMesh.position.set(0, 0.08, 0.0051);
      lidGroup.add(displayMesh);

      // Little glowing Apple logo on back
      const logoMesh = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.03, 0.002), new THREE.MeshBasicMaterial({ color: 0xffffff }));
      logoMesh.position.set(0, 0.08, -0.0051);
      lidGroup.add(logoMesh);

      // Angle the lid open (tilted back)
      lidGroup.rotation.x = 0.35; // 0.35 radians open angle
    ...
    // Oscar's MacBook Pro (replacing old desktop PC)
    const oscarMacbook = createMacbook(0x3b82f6); // Blue blueprint style display
    oscarMacbook.position.set(0, 0.5, 0.05);
    oscarDeskGroup.add(oscarMacbook);
    ...
    // Sheldon's MacBook Pro (replacing old desktop PC)
    const sheldonMacbook = createMacbook(0x38bdf8); // Cyan developer style display
    sheldonMacbook.position.set(0, 0.5, 0.05);
    sheldonDeskGroup.add(sheldonMacbook);
    ...
    // Laptop Group rotated 180 degrees to face Nairobi's couch correctly
    const laptopGroup = new THREE.Group();
    laptopGroup.position.set(0, 0.13, 0);
    laptopGroup.rotation.y = Math.PI;
    coffeeTable.add(laptopGroup);
    ...
    laptopScreen.position.set(0, 0.05, -0.05);
    ...
    // Ragnar's MacBook Pro (replacing old PC, rotated 180 degrees to face Ragnar)
    const ragnarMacbook = createMacbook(0x10b981); // Green terminal style display
    ragnarMacbook.position.set(0, 0.5, 0.05);
    ragnarMacbook.rotation.y = Math.PI;
    ragnarDeskGroup.add(ragnarMacbook);

    // Connected Apple Studio Display for Ragnar (rotated 180 degrees to face Ragnar)
    const studioDisplayGroup = new THREE.Group();
    studioDisplayGroup.position.set(0, 0, -0.15);
    studioDisplayGroup.rotation.y = Math.PI;
    ragnarDeskGroup.add(studioDisplayGroup);

    const studioDisplayBase = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.015, 0.12), macbookSilverMat);
    studioDisplayBase.position.set(0, 0.508, 0);
    studioDisplayGroup.add(studioDisplayBase);

    const studioDisplayStand = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.22, 0.03), macbookSilverMat);
    studioDisplayStand.position.set(0, 0.62, 0);
    studioDisplayGroup.add(studioDisplayStand);

    const studioDisplayScreen = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.26, 0.015), macbookSilverMat);
    studioDisplayScreen.position.set(0, 0.75, 0);
    studioDisplayGroup.add(studioDisplayScreen);

    const studioDisplayPanel = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.24, 0.005), new THREE.MeshBasicMaterial({ color: 0xfacc15 })); // Yellow dashboard display
    studioDisplayPanel.position.set(0, 0.75, 0.008);
    studioDisplayGroup.add(studioDisplayPanel);
    ...
    stateRef.current.vfxObjects = [particleSystem, orbitRing1, orbitRing2];
    ...
    const onMouseClick = (e) => {
      ...
      if (clickedAgent) {
        ...
        playSynthSound('click', clickedAgent);
    ...
    const animate = () => {
      ...
      if (active && AGENTS[active]) {
        ...
        speakerLight.intensity = 3.0 + Math.sin(Date.now() * 0.015) * 1.0;
      ...
      agentsArray.forEach(id => {
        ...
        if (shoulderL && shoulderR && elbowL && elbowR) {
          const isTyping = isAtNormalPos && !isCoffee && stateRef.current.running;
          ...
          // Pose transition for typing (transition between idle/walk arms and typing arms)
          const typePoseTarget = isTyping ? 1.0 : 0.0;
          av.typeTransition = av.typeTransition ?? 0;
          av.typeTransition += (typePoseTarget - av.typeTransition) * 0.15;
          const tp = av.typeTransition;

          // Left Arm default walking/sitting rotation
          const defaultShoulderLx = -0.4 * (1 - t) + (0.1 - wSwing * 0.3) * t;
          const defaultElbowLx = 1.0 * (1 - t) + 0.25 * t;

          // Left Arm typing pose rotation (raised and bent towards the keys)
          const typingShoulderLx = -0.45 + typeSwingL;
          const typingElbowLx = 1.0 + typeSwingL * 0.5;

          // Interpolated Left Arm rotations
          shoulderL.rotation.x = (1 - tp) * defaultShoulderLx + tp * typingShoulderLx;
          shoulderL.rotation.z = 0.05 * (1 - t) + 0.08 * t;
          elbowL.rotation.x = (1 - tp) * defaultElbowLx + tp * typingElbowLx;

          // Right Arm default walking/sitting rotation
          const defaultShoulderRx = -0.4 * (1 - t) - 0.5 * t;
          const defaultElbowRx = 1.0 * (1 - t) + 1.2 * t;

          // Right Arm typing pose rotation (raised and bent towards the keys)
          const typingShoulderRx = -0.45 + typeSwingR;
          const typingElbowRx = 1.0 + typeSwingR * 0.5;

          // Interpolated Right Arm rotations
          shoulderR.rotation.x = (1 - tp) * defaultShoulderRx + tp * typingShoulderRx;
          shoulderR.rotation.y = 0 * (1 - t) - 0.3 * t;
          shoulderR.rotation.z = -0.05 * (1 - t) - 0.1 * t;
          elbowR.rotation.x = (1 - tp) * defaultElbowRx + tp * typingElbowRx;
        }
      ...
      if (stateRef.current.vfxObjects) {
        stateRef.current.vfxObjects[0].rotation.y += 0.001; // Particles
        stateRef.current.vfxObjects[1].rotation.z += 0.01;  // Ring 1
        stateRef.current.vfxObjects[2].rotation.z -= 0.008; // Ring 2

        // Ensure visibility is synced (since stateRef captures the latest)
        const isVfxOn = stateRef.current.vfxEnabled;
        stateRef.current.vfxObjects.forEach(obj => obj.visible = isVfxOn);
      }
    ...
    return () => {
      ...
      // Proper Three.js disposal to prevent GPU memory leaks
      scene.traverse((obj) => {
      ...
      if (viewportRef.current && renderer.domElement.parentNode === viewportRef.current) {
