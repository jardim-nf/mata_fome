// src/pages/admin/PdvScreen.jsx - VERSÃO COM SUPORTE A VARIAÇÕES
...
const PdvScreen = () => {
  ...
  // --- NOVO: ESTADO PARA SELEÇÃO DE VARIAÇÃO ---
      ...
      catSnap.forEach(catDoc => {
        ...
        const unsubscribeItems = onSnapshot(qItems, (itemsSnap) => {
          const itemsDaCategoria = itemsSnap.docs.map(itemDoc => {
             ...
             // Se tiver variações ativas, define o preço base como o menor preço encontrado para exibição "A partir de"
             ...
             return {
                ...
                price: precoBase,
                name: data.nome,
                variacoes: variacoes, // Guarda as variações limpas
  ...
  // --- NOVA LÓGICA DE CLIQUE NO PRODUTO ---
  const handleProdutoClick = (produto) => {
    if (!vendaAtual) { alert('Inicie uma venda primeiro (F2)'); return; }

    // 1. Se tem variações reais (mais de uma ou nome diferente de Padrão)
    if (produto.temVariacoes && produto.variacoes.length > 0) {
        // Se tiver só 1 variação e for "Padrão" ou "Tamanho Único", adiciona direto
        if (produto.variacoes.length === 1 && (produto.variacoes[0].nome === 'Padrão' || produto.variacoes[0].nome === 'Único')) {
            ...
            // Se tiver várias, abre modal
            setProdutoParaSelecao(produto);
        }
    } else {
        // Produto simples sem variação (usa preço base)
  ...
  const adicionarItemAoCarrinho = (produto, variacaoSelecionada) => {
    setVendaAtual(prev => {
      // Cria ID único baseado no produto E na variação
      ...
      if (itemExistente) {
        ...
        novosItens = [...prev.itens, {
            id: Date.now().toString(), // ID único da linha no carrinho
            uniqueItemId: uniqueItemId, // ID para agrupar (Prod + Var)
    ...
    setProdutoParaSelecao(null); // Fecha modal se estiver aberto
  ...
  const cancelarVenda = () => {
    if (vendaAtual?.itens.length > 0 && !confirm('Cancelar venda?')) return;
    setVendaAtual(null);
    setMostrarFinalizacao(false);
  };
  ...
  // NOVO MODAL DE SELEÇÃO DE VARIAÇÃO
            ...
            <div className="bg-white rounded-2xl shadow-xl max-w-lg flex-col sm:flex-row overflow-hidden animate-fade-in-up">
                ...
                <div className="p-6">
                    ...
                    <div className="grid gap-3">
                        {produtoParaSelecao.variacoes.map((v) => (
                            <button
                                key={v.id}
                                onClick={() => adicionarItemAoCarrinho(produtoParaSelecao, v)}
        ...
        <div className="bg-white flex-col sm:flex-row max-w-sm shadow-2xl overflow-hidden relative animate-bounce-in">
          ...
          <div className="p-8 pt-10 font-mono text-sm text-gray-700">
            <div className="text-center mb-6 border-b-2 border-dashed border-gray-300 pb-4">
              ...
              <p className="text-xs text-gray-400 mt-1">ID: {dadosRecibo.id?.slice(-6).toUpperCase()}</p>
            </div>
            <div className="space-y-3 mb-6 max-h-40 overflow-y-auto custom-scrollbar">
              {dadosRecibo.itens.map((item, index) => (
                <div key={index} className="flex justify-between items-start border-b border-gray-100 pb-2">
                  <div><span className="font-bold mr-2">{item.quantidade}x</span>{item.nome}</div>
                  <div className="font-semibold">R$ {(item.preco * item.quantidade).toFixed(2)}</div>
                </div>
              ))}
            </div>
            <div className="border-t-2 border-dashed border-gray-300 pt-4 space-y-2 mb-6">
              <div className="flex justify-between text-xl font-bold text-gray-900"><span>TOTAL</span><span>R$ {dadosRecibo.total.toFixed(2)}</span></div>
              <div className="flex justify-between text-sm mt-2 bg-gray-50 p-2 rounded"><span>Pagamento:</span><span className="uppercase font-bold">{dadosRecibo.formaPagamento}</span></div>
              {dadosRecibo.troco > 0 && <div className="flex justify-between text-sm px-2 text-green-700 font-bold"><span>Troco:</span><span>R$ {dadosRecibo.troco.toFixed(2)}</span></div>}
            </div>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md flex-col sm:flex-row">
          ...
          <div className="p-6 border-b">
            ...
            <div className="grid grid-cols-2 gap-3">
              {['dinheiro', 'cartao', 'pix', 'debito'].map(f => (
                <button key={f} onClick={() => { setFormaPagamento(f); if(f!=='dinheiro') setValorRecebido(''); }} className={`p-3 rounded-xl border-2 capitalize font-bold ${formaPagamento===f?'border-green-500 bg-green-50 text-green-700':'border-gray-200'}`}>{f}</button>
              ))}
            </div>
          </div>
          ...
          <div className="p-6 flex gap-3">
            <button onClick={() => setMostrarFinalizacao(false)} className="flex-1 bg-gray-200 py-3 rounded-xl font-bold">Voltar</button>
            <button onClick={finalizarVenda} disabled={salvando} className="flex-1 bg-green-600 text-white py-3 rounded-xl font-bold hover:bg-green-700">{salvando?'...':'Confirmar (F10)'}</button>
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm">
            ...
            {!vendaAtual ? (
              <button onClick={iniciarVendaBalcao} className="bg-green-600 text-white px-6 py-3 rounded-xl font-bold shadow hover:bg-green-700">🛒 Abrir Caixa (F2)</button>
            ) : <span className="bg-blue-100 text-blue-800 px-4 py-2 rounded-lg font-bold">Venda Aberta</span>}
          </div>
          ...
          {carregandoProdutos ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4 h-[60vh] overflow-y-auto content-start pr-2 custom-scrollbar">
              {produtosFiltrados.map(produto => (
                ...
                >
                  <div className="text-3xl mb-2 bg-gray-50 w-12 h-12 flex items-center justify-center rounded-lg">{getIconeCategoriaDinamico(produto.categoria)}</div>
                    ...
                    <div key={item.uniqueItemId || item.id} className="flex justify-between items-start group border-b border-dashed border-gray-100 pb-2">
                      ...
                      <div className="text-right">
                        ...
                        <div className="flex gap-1 justify-end mt-1 opacity-0 group-hover:opacity-100 transition">
                          <button onClick={() => ajustarQuantidade(item.id, item.quantity - 1)} className="w-6 h-6 bg-gray-200 rounded text-gray-600 hover:bg-gray-300">-</button>
                          <button onClick={() => ajustarQuantidade(item.id, item.quantity + 1)} className="w-6 h-6 bg-gray-200 rounded text-gray-600 hover:bg-gray-300">+</button>
                          <button onClick={() => removerItem(item.id)} className="w-6 h-6 bg-red-100 text-red-600 rounded">x</button>
                        </div>
                      </div>
                    </div>
                  ))
                ) : <div className="text-center text-gray-400 py-10">Carrinho vazio</div>
              ) : <div className="text-center text-gray-400 py-10 flex flex-col items-center h-full justify-center"><span className="text-6xl mb-4">🏪</span><p>Caixa Fechado</p></div>}
            </div>
            {vendaAtual && (
              <div className="p-4 bg-gray-50 border-t rounded-b-2xl space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button className="bg-purple-100 text-purple-700 py-3 rounded-xl font-bold">NFC-e (F12)</button>
                  <button onClick={cancelarVenda} className="bg-red-100 text-red-700 py-3 rounded-xl font-bold">Cancelar</button>
                  <button onClick={() => setMostrarFinalizacao(true)} className="col-span-2 bg-green-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-green-700 shadow-lg">Finalizar Venda (F10)</button>
                </div>
              </div>
            )}
          </div>
