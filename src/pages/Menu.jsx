...
// 🔥 UTILITÁRIO CSS PARA O MÓDULO (Sem tags <style> globais perigosas)
const scrollbarHideClass = "scrollbar-hide"; // Assumindo que você tem essa classe no CSS global, senão use overflow-hidden
...
function Menu() {
    ...
    const [coresEstabelecimento, setCoresEstabelecimento] = useState({
        ...
        destaque: '#059669',
        background: '#f9fafb',
        texto: { principal: '#111827', destaque: '#FBBF24' }
    });

    // ... (Funções auxiliares iguais, omitidas para brevidade, mas mantidas na lógica) ...
    // Vou reincluir as funções principais para garantir que nada quebre.
    ...
    const cleanData = (obj) => { /* Mesma lógica de cleanData */ 
        if (obj === null || obj === undefined) return obj;
        return Object.entries(obj).reduce((acc, [key, value]) => {
            if (value === undefined) acc[key] = null;
            else if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
                if (value instanceof Date || (value.seconds !== undefined) || ('_methodName' in value)) acc[key] = value;
                else acc[key] = cleanData(value);
            } else if (Array.isArray(value)) acc[key] = value.map(item => typeof item === 'object' ? cleanData(item) : item);
            else acc[key] = value;
            return acc;
        }, {});
    };
    ...
    const taxaAplicada = useMemo(() => (isRetirada || premioRaspadinha?.type === 'frete') ? 0 : taxaEntregaCalculada, [isRetirada, taxaEntregaCalculada, premioRaspadinha]);
    const finalOrderTotal = useMemo(() => {
        let total = subtotalCalculado + taxaAplicada - discountAmount;
        if (premioRaspadinha?.type === 'desconto') total -= subtotalCalculado * (premioRaspadinha.valor / 100);
        return Math.max(0, total);
    }, [subtotalCalculado, taxaAplicada, discountAmount, premioRaspadinha]);

    // Lógica IA e Produtos
    const carregarProdutosRapido = async (estabId) => {
        try {
            ...
            const promessas = categoriasSnapshot.docs.map(async (catDoc) => {
                ...
                return itensSnapshot.docs.map(itemDoc => ({ ...itemDoc.data(), id: itemDoc.id, categoria: categoriaData.nome || 'Geral', categoriaId: catDoc.id }));
            });
            const resultados = await Promise.all(promessas);
            return resultados.flat().filter(item => item.preco !== undefined || item.precoFinal !== undefined || (item.variacoes && item.variacoes.length > 0));
        } catch (error) { console.error("Erro ao carregar:", error); return []; }
    };
    ...
    const handleAdicionarPorIA = useCallback((comandoCompleto) => {
        ...
        } else if (produtoEncontrado.variacoes?.length === 1) variacaoSelecionada = produtoEncontrado.variacoes[0];
    ...
    const handleConfirmarVariacoes = (item) => {
        const preco = Number(item.variacaoSelecionada?.preco || item.preco);
        setCarrinho(prev => [...prev, { ...item, qtd: 1, cartItemId: uuidv4(), precoFinal: preco }]);
        setItemParaVariacoes(null);
    };

    const handleConfirmarAdicionais = (item) => {
        const preco = Number(item.precoFinal || item.preco);
        setCarrinho(prev => [...prev, { ...item, qtd: 1, cartItemId: uuidv4(), precoFinal: preco }]);
        setItemParaAdicionais(null);
    };

    const removerDoCarrinho = (id) => {
        const item = carrinho.find(p => p.cartItemId === id);
        if (!item) return;
        if (item.qtd === 1) setCarrinho(carrinho.filter(p => p.cartItemId !== id));
        else setCarrinho(carrinho.map(p => p.cartItemId === id ? { ...p, qtd: p.qtd - 1 } : p));
    };
    ...
    const handleApplyCoupon = async () => {
        ...
        } catch (e) { toast.error(e.message); } finally { setCouponLoading(false); }
    };

    const prepararParaPagamento = () => {
        if (!currentUser) return handleAbrirLogin();
        if (carrinho.length === 0) return toast.warn('Carrinho vazio.');
        const itensFormatados = carrinho.map(item => ({ nome: formatarItemCarrinho(item), quantidade: item.qtd, preco: Number(item.precoFinal) }));
        const pedidoRaw = {
    ...
    const handlePagamentoSucesso = async (result) => {
        ...
        } catch (e) { console.error(e); } finally { setProcessandoPagamento(false); }
    };
    ...
    if (currentUser && (isAdmin || isMasterAdmin)) return <div className="p-10 text-center">Acesso Admin.<button onClick={handleLogout} className="block mx-auto mt-4 bg-red-600 text-white p-2 rounded">Sair</button></div>;

    return (
        <div className="w-full relative min-h-screen text-left overflow-x-hidden" style={{ backgroundColor: coresEstabelecimento.background, color: coresEstabelecimento.texto.principal, paddingBottom: '150px' }}>
            <div className="max-w-7xl mx-auto px-4 w-full">
                ...
                {estabelecimentoInfo && (
                    <div className="bg-white rounded-xl p-4 md:p-6 mb-6 mt-6 border flex flex-col md:flex-row gap-4 md:gap-6 items-center shadow-lg relative text-center md:text-left">
                        <div className="absolute top-3 right-3 z-10 md:top-4 md:right-4">
                             {currentUser ? (
                                <button onClick={handleLogout} className="flex items-center gap-2 text-xs md:text-sm text-red-500 bg-red-50 px-3 py-1.5 rounded-full border border-red-100 hover:bg-red-100">
                                    <IoLogOutOutline size={18} /> Sair
                                </button>
                             ) : (
                                <button type="button" onClick={handleAbrirLogin} className="flex items-center gap-2 text-xs md:text-sm font-bold text-white px-3 py-1.5 rounded-full shadow-md" style={{ backgroundColor: coresEstabelecimento.destaque }}>
                                    <IoPerson size={16} /> Entrar
                                </button>
                             )}
                        </div>
                ...
                <div className="bg-white p-3 md:p-4 mb-6 sticky top-0 z-30 shadow-sm rounded-b-xl md:rounded-lg border-b border-gray-100">
                    <input type="text" placeholder="🔍 Buscar..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full p-3 mb-3 border rounded-full bg-gray-50 text-base text-gray-900 focus:bg-white focus:ring-2 outline-none" style={{ focusRing: coresEstabelecimento.destaque }} />
                    <div className="flex overflow-x-auto gap-2 pb-1 w-full">
                        {['Todos', ...categoriasOrdenadas].map(cat => (
                            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all shadow-sm ${selectedCategory === cat ? 'text-white' : 'bg-gray-100 text-gray-600'}`} style={{ backgroundColor: selectedCategory === cat ? coresEstabelecimento.destaque : undefined }}>{cat}</button>
                        ))}
                    </div>
                </div>
                ...
                {categoriasOrdenadas.map(cat => {
                    ...
                    return (
                        <div key={cat} id={`categoria-${cat}`} className="mb-8">
                            ...
                            {items.length > 4 && <button onClick={() => visible >= items.length ? handleShowLess(cat) : handleShowMore(cat)} className="w-full mt-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-500 font-bold">{visible >= items.length ? 'Ver menos' : 'Ver mais'}</button>}
                        </div>
                    );
                })}

                {/* Formulários e Resumo */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mt-12 pb-10">
                    <div className="bg-white p-5 md:p-6 rounded-xl border shadow-lg text-left order-2 md:order-1">
                        ...
                        {/* Formulários com text-base para evitar zoom */}
                        <div className="space-y-3">
                            <input className="w-full p-3 rounded-lg border bg-gray-50 text-base text-gray-900 outline-none focus:bg-white" placeholder="Nome *" value={nomeCliente} onChange={e => setNomeCliente(e.target.value)} />
                            <input className="w-full p-3 rounded-lg border bg-gray-50 text-base text-gray-900 outline-none focus:bg-white" placeholder="Telefone *" value={telefoneCliente} onChange={e => setTelefoneCliente(e.target.value)} type="tel" />
                            {!isRetirada && (
                                <div className="space-y-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                                    <div className="flex gap-2">
                                        ...
                                        <input className="w-24 p-3 rounded-lg border bg-white text-base text-center text-gray-900" placeholder="Nº *" value={numero} onChange={e => setNumero(e.target.value)} />
                                    </div>
                            ...
                            <div className="flex gap-2 pt-2">
                                <button onClick={() => setIsRetirada(false)} className={`flex-1 p-3 rounded-xl font-bold border-2 ${!isRetirada ? 'bg-green-50 border-green-500 text-green-700' : 'bg-white border-gray-200 text-gray-400'}`}>🚚 Entrega</button>
                                <button onClick={() => setIsRetirada(true)} className={`flex-1 p-3 rounded-xl font-bold border-2 ${isRetirada ? 'bg-green-50 border-green-500 text-green-700' : 'bg-white border-gray-200 text-gray-400'}`}>🏪 Retirada</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="resumo-carrinho" className="bg-white p-5 md:p-6 rounded-xl border shadow-lg text-left text-gray-900 order-1 md:order-2 h-fit">
                        ...
                        {carrinho.length === 0 ? <div className="text-center py-10 bg-gray-50 text-gray-400">Vazio</div> : (
                            <>
                                <div className="space-y-3 mb-4 max-h-60 overflow-y-auto">
                                    {carrinho.map(item => (
                                        <div key={item.cartItemId} className="flex justify-between items-start border-b pb-2">
                                            <div className="flex-1 pr-2"><p className="font-bold text-sm">{formatarItemCarrinho(item)}</p><p className="text-xs text-gray-500">R$ {item.precoFinal.toFixed(2)} x {item.qtd}</p></div>
                                            <div className="flex items-center gap-3"><span className="font-bold text-sm">R$ {(item.precoFinal * item.qtd).toFixed(2)}</span><button onClick={() => removerDoCarrinho(item.cartItemId)} className="text-red-500 font-bold">✕</button></div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <button onClick={prepararParaPagamento} className="w-full mt-6 py-4 rounded-xl font-bold text-lg text-white shadow-lg bg-green-600">✅ Finalizar</button>
                            </>
                        )}
                    </div>
                </div>
            ...
            {estabelecimentoInfo && <AIChatAssistant estabelecimento={estabelecimentoInfo} produtos={allProdutos} onAddDirect={handleAdicionarPorIA} onCheckout={scrollToResumo} mode={showAICenter ? "center" : "widget"} onClose={() => setShowAICenter(false)} clienteNome={nomeCliente} onRequestLogin={handleAbrirLogin} carrinho={carrinho} onClick={prepararParaPagamento} />}
            <AIWidgetButton />
            ...
            {showOrderConfirmationModal && <div className="fixed inset-0 bg-black/80 z-[5000] flex items-center justify-center p-4"><div className="bg-white p-8 rounded-2xl text-center shadow-2xl"><h2 className="text-3xl font-bold mb-4">Pedido Recebido!</h2><button onClick={() => setShowOrderConfirmationModal(false)} className="w-full bg-green-600 text-white py-3 rounded-xl font-bold">Fechar</button></div></div>}
            {showRaspadinha && <RaspadinhaModal onGanhar={handleGanharRaspadinha} onClose={() => setShowRaspadinha(false)} />}
            
            {/* Modal Login (Mobile Safe) */}
            {showLoginPrompt && (
                <div className="fixed inset-0 z-[5000] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white p-6 rounded-2xl w-full max-w-md relative text-left shadow-2xl max-h-[90vh] overflow-y-auto">
                        <button onClick={() => setShowLoginPrompt(false)} className="absolute top-4 right-4 text-2xl text-gray-400 hover:text-gray-800">&times;</button>
                        <h2 className="text-2xl font-bold text-center mb-6">{isRegisteringInModal ? 'Criar Conta' : 'Login'}</h2>
                        <form onSubmit={isRegisteringInModal ? handleRegisterModal : handleLoginModal} className="space-y-4">
                            {isRegisteringInModal && <><input placeholder="Nome" value={nomeAuthModal} onChange={e => setNomeAuthModal(e.target.value)} className="w-full p-3 rounded border text-base" required /><input placeholder="Telefone" value={telefoneAuthModal} onChange={e => setTelefoneAuthModal(e.target.value)} className="w-full p-3 rounded border text-base" required type="tel" /></>}
                            <input type="email" placeholder="Email" value={emailAuthModal} onChange={e => setEmailAuthModal(e.target.value)} className="w-full p-3 rounded border text-base" required />
                            <input type="password" placeholder="Senha" value={passwordAuthModal} onChange={e => setPasswordAuthModal(e.target.value)} className="w-full p-3 rounded border text-base" required />
                        ...
                        <button onClick={() => setIsRegisteringInModal(!isRegisteringInModal)} className="w-full mt-4 text-green-600 text-sm font-bold text-center block">{isRegisteringInModal ? 'Já tem conta? Entrar' : 'Criar conta'}</button>
                    </div>
                </div>
            )}
        </div>
