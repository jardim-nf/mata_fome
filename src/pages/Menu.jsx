...
function Menu() {
  ...
  // Seus estados existentes (sem alterações)
  ...
  // ▼▼▼ ESTADO MODIFICADO ▼▼▼
  // Agora 'expandedCategories' guarda o NÚMERO de itens visíveis por categoria
  ...
  // ▼▼▼ NOVAS FUNÇÕES PARA CONTROLAR A VISIBILIDADE DOS ITENS ▼▼▼
  ...
  // Seus useEffects existentes (sem alterações, exceto o de buscar produtos)
  useEffect(() => {
    const handler = setTimeout(() => { setDebouncedSearchTerm(searchTerm); }, 300);
    return () => { clearTimeout(handler); };
  }, [searchTerm]);

  useEffect(() => { if (authLoading === false) { if (currentUser === null) { setShowLoginPrompt(true); } else { setShowLoginPrompt(false); } } }, [authLoading, currentUser]);
  
  useEffect(() => { if (!authLoading) { if (currentUser && currentClientData) { setNomeCliente(currentClientData.nome || ''); setTelefoneCliente(currentClientData.telefone || ''); if (currentClientData.endereco) { setRua(currentClientData.endereco.rua || ''); setNumero(currentClientData.endereco.numero || ''); setBairro(currentClientData.endereco.bairro || ''); setCidade(currentClientData.endereco.cidade || ''); setComplemento(currentClientData.endereco.complemento || ''); setIsRetirada(false); } else { setIsRetirada(true); } } else { const storedNome = localStorage.getItem('nomeCliente') || ''; const storedTelefone = localStorage.getItem('telefoneCliente') || ''; const storedRua = localStorage.getItem('rua') || ''; const storedNumero = localStorage.getItem('numero') || ''; const storedBairro = localStorage.getItem('bairro') || ''; const storedCidade = localStorage.getItem('cidade') || ''; const storedComplemento = localStorage.getItem('complemento') || ''; setNomeCliente(storedNome); setTelefoneCliente(storedTelefone); setRua(storedRua); setNumero(storedNumero); setBairro(storedBairro); setCidade(storedCidade); setComplemento(storedComplemento); if (storedRua && storedNumero && storedBairro && storedCidade) { setIsRetirada(false); } else { setIsRetirada(true); } } } }, [currentUser, currentClientData, authLoading]);
  
  useEffect(() => { const taxasRef = collection(db, 'taxasDeEntrega'); const q = query(taxasRef, orderBy('nomeBairro')); const unsubscribe = onSnapshot(q, (snapshot) => { const data = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })); setTaxasBairro(data); }, (error) => { console.error("Erro ao carregar taxas de entrega:", error); toast.error("Erro ao carregar taxas de entrega."); }); return () => unsubscribe(); }, []);
  
  useEffect(() => { if (isRetirada) { setTaxaEntregaCalculada(0); setBairroNaoEncontrado(false); return; } if (bairro.trim() === "" || cidade.trim() === "") { setTaxaEntregaCalculada(0); setBairroNaoEncontrado(false); return; } const bairroEncontrado = taxasBairro.find((taxa) => taxa.nomeBairro.toLowerCase() === bairro.trim().toLowerCase()); if (bairroEncontrado) { setTaxaEntregaCalculada(bairroEncontrado.valorTaxa); setBairroNaoEncontrado(false); } else { setTaxaEntregaCalculada(0); setBairroNaoEncontrado(true); } }, [bairro, cidade, taxasBairro, isRetirada]);
      ...
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          if (selectedCategory && selectedCategory !== 'Todos') {
            produtosDaSubcolecao = produtosDaSubcolecao.filter(item => item.categoria && item.categoria.toLowerCase() === selectedCategory.toLowerCase());
          }
          if (debouncedSearchTerm.trim() !== '') {
            const lowerCaseSearchTerm = debouncedSearchTerm.trim().toLowerCase();
            produtosDaSubcolecao = produtosDaSubcolecao.filter(item => (item.nome && item.nome.toLowerCase().includes(lowerCaseSearchTerm)) || (item.descricao && item.descricao.toLowerCase().includes(lowerCaseSearchTerm)));
          }
          produtosDaSubcolecao.sort((a, b) => (a.nome || '').localeCompare(b.nome || ''));
          setProdutos(produtosDaSubcolecao);
        } else {
        ...
        console.error("Erro ao carregar estabelecimento ou cardápio:", error);
  ...
  useEffect(() => { const storedReorderItems = localStorage.getItem('reorderItems'); if (storedReorderItems) { try { const parsedItems = JSON.parse(storedReorderItems); if (Array.isArray(parsedItems) && parsedItems.length > 0) { setCarrinho(prevCarrinho => { const newCarrinho = [...prevCarrinho]; parsedItems.forEach(reorderItem => { const existing = newCarrinho.find(item => item.id === reorderItem.id); if (existing) { existing.qtd += reorderItem.qtd; } else { newCarrinho.push(reorderItem); } }); return newCarrinho; }); toast.success('Seu pedido anterior foi carregado no carrinho!'); } } catch (e) { console.error("Erro ao parsear reorderItems do localStorage:", e); toast.error('Erro ao carregar re-pedido do histórico.'); } finally { localStorage.removeItem('reorderItems'); } } }, []);
  
  // Suas outras funções (handleAbrirModalAdicionais, enviarPedido, etc. sem alterações)
  const handleAbrirModalAdicionais = (item) => { if (item.adicionais && Array.isArray(item.adicionais) && item.adicionais.length > 0) { setItemParaAdicionais(item); } else { adicionarItemSimplesAoCarrinho(item); } };
  const handleFecharModal = () => { setItemParaAdicionais(null); };
  const adicionarItemSimplesAoCarrinho = (item) => { if (!currentUser) { toast.warn('Para adicionar itens, por favor, faça login ou cadastre-se.'); setShowLoginPrompt(true); return; } const itemExistente = carrinho.find(p => p.id === item.id && (!p.adicionais || p.adicionais.length === 0)); if (itemExistente) { setCarrinho(carrinho.map(p => p.cartItemId === itemExistente.cartItemId ? { ...p, qtd: p.qtd + 1 } : p)); } else { const novoItemNoCarrinho = { ...item, qtd: 1, cartItemId: uuidv4(), precoFinal: item.preco, adicionais: [] }; setCarrinho([...carrinho, novoItemNoCarrinho]); } toast.success(`${item.nome} adicionado ao carrinho!`); };
  const handleConfirmarAdicionais = (itemConfigurado) => { const novoItemNoCarrinho = { ...itemConfigurado, qtd: 1, cartItemId: uuidv4() }; setCarrinho(prevCarrinho => [...prevCarrinho, novoItemNoCarrinho]); toast.success(`${itemConfigurado.nome} foi adicionado ao carrinho!`); handleFecharModal(); };
  const removerDoCarrinho = (cartItemId) => { const produtoNoCarrinho = carrinho.find((p) => p.cartItemId === cartItemId); if (!produtoNoCarrinho) return; if (produtoNoCarrinho.qtd === 1) { setCarrinho(carrinho.filter((p) => p.cartItemId !== cartItemId)); toast.info(`${produtoNoCarrinho.nome} removido do carrinho.`); } else { setCarrinho(carrinho.map((p) => (p.cartItemId === cartItemId ? { ...p, qtd: p.qtd - 1 } : p))); toast.info(`Quantidade de ${produtoNoCarrinho.nome} reduzida.`); } };
  const handleApplyCoupon = async () => { if (!currentUser) { toast.warn('Você precisa estar logado para aplicar um cupom.'); return; } if (!couponCodeInput.trim()) { toast.warn('Por favor, digite o código do cupom.'); return; } setCouponLoading(true); setAppliedCoupon(null); setDiscountAmount(0); try { const couponsRef = collection(db, 'cupons'); const q = query(couponsRef, where('codigo', '==', couponCodeInput.toUpperCase().trim())); const couponSnap = await getDocs(q); if (couponSnap.empty) { toast.error('Cupom inválido ou não encontrado.'); setCouponLoading(false); return; } const couponDoc = couponSnap.docs[0]; const couponData = { id: couponDoc.id, ...couponDoc.data() }; const now = Timestamp.now(); if (!couponData.ativo) { toast.error('Cupom inativo.'); setCouponLoading(false); return; } if (couponData.validadeInicio && couponData.validadeInicio.seconds > now.seconds) { toast.error('Cupom ainda não válido.'); setCouponLoading(false); return; } if (couponData.validadeFim && couponData.validadeFim.seconds < now.seconds) { toast.error('Cupom expirado.'); setCouponLoading(false); return; } if (couponData.usosMaximos !== null && couponData.usosAtuais >= couponData.usosMaximos) { toast.error('Cupom atingiu o limite máximo de usos.'); setCouponLoading(false); return; } if (couponData.minimoPedido !== null && subtotalCalculado < couponData.minimoPedido) { toast.error(`Pedido mínimo de R$ ${couponData.minimoPedido.toFixed(2).replace('.', ',')} para usar este cupom.`); setCouponLoading(false); return; } if (couponData.estabelecimentosId && couponData.estabelecimentosId.length > 0 && !couponData.estabelecimentosId.includes(actualEstabelecimentoId)) { toast.error('Este cupom não é válido para este estabelecimento.'); setCouponLoading(false); return; } if (couponData.usosPorUsuario !== null) { const userCouponUsageRef = doc(db, 'clientes', currentUser.uid, 'couponUsage', couponData.id); const userCouponUsageSnap = await getDocFirestore(userCouponUsageRef); if (userCouponUsageSnap.exists() && userCouponUsageSnap.data().count >= couponData.usosPorUsuario) { toast.error('Você já usou este cupom o número máximo de vezes.'); setCouponLoading(false); return; } } let calculatedDiscount = 0; if (couponData.tipoDesconto === 'percentual') { calculatedDiscount = subtotalCalculado * (couponData.valorDesconto / 100); } else if (couponData.tipoDesconto === 'valorFixo') { calculatedDiscount = couponData.valorDesconto; if (calculatedDiscount > subtotalCalculado) { calculatedDiscount = subtotalCalculado; } } else if (couponData.tipoDesconto === 'freteGratis') { calculatedDiscount = taxaAplicada; } setAppliedCoupon(couponData); setDiscountAmount(calculatedDiscount); toast.success(`Cupom ${couponData.codigo} aplicado! Desconto de R$ ${calculatedDiscount.toFixed(2).replace('.', ',')}.`); setCouponLoading(false); } catch (error) { console.error("Erro ao aplicar cupom:", error); toast.error('Erro ao aplicar cupom. Tente novamente.'); setCouponLoading(false); setAppliedCoupon(null); setDiscountAmount(0); } };
  const removeAppliedCoupon = () => { setAppliedCoupon(null); setDiscountAmount(0); setCouponCodeInput(''); toast.info('Cupom removido.'); };
  const enviarPedido = async () => { if (!currentUser) { toast.warn('Você precisa estar logado para enviar um pedido.'); setShowLoginPrompt(true); return; } if (!actualEstabelecimentoId) { toast.error('Erro: Estabelecimento não carregado corretamente. Por favor, recarregue a página.'); return; } if (!nomeCliente.trim() || !telefoneCliente.trim() || carrinho.length === 0 || !formaPagamento) { toast.warn('Por favor, preencha todos os seus dados (Nome, Telefone), adicione itens ao carrinho e selecione uma forma de pagamento.'); return; } if (!isRetirada && (!rua.trim() || !numero.trim() || !bairro.trim() || !cidade.trim())) { toast.warn('Para entrega, por favor, preencha o endereço completo (Rua, Número, Bairro, Cidade).'); return; } if (!isRetirada && bairroNaoEncontrado && taxaEntregaCalculada === 0) { const confirmacao = window.confirm(`O bairro "${bairro.trim()}" não foi encontrado na nossa lista de áreas de entrega e a taxa é R$ 0,00. Deseja continuar? (Podem ser aplicadas taxas adicionais na entrega)`); if (!confirmacao) { return; } } let valorTrocoPara = null; if (formaPagamento === 'dinheiro' && trocoPara.trim() !== '') { const trocoNum = Number(trocoPara); if (trocoNum > finalOrderTotal) { valorTrocoPara = trocoNum; } else { toast.warn(`O valor para troco (R$ ${trocoNum.toFixed(2).replace('.', ',')}) deve ser maior que o total do pedido (R$ ${finalOrderTotal.toFixed(2).replace('.', ',')}).`); return; } } const pedido = { cliente: { nome: nomeCliente.trim(), telefone: telefoneCliente.trim(), endereco: isRetirada ? null : { rua: rua.trim(), numero: numero.trim(), bairro: bairro.trim(), cidade: cidade.trim(), complemento: complemento.trim() || null }, userId: currentUser.uid }, estabelecimentoId: actualEstabelecimentoId, itens: carrinho.map(item => ({ nome: item.nome, quantidade: item.qtd, preco: Number(item.precoFinal), imageUrl: item.imageUrl || null, adicionais: item.adicionais || [] })), status: 'recebido', criadoEm: Timestamp.now(), formaPagamento: formaPagamento, trocoPara: valorTrocoPara, taxaEntrega: taxaAplicada, totalFinal: finalOrderTotal, tipoEntrega: isRetirada ? 'retirada' : 'delivery', ...(formaPagamento === 'pix' && { statusPagamentoPix: 'aguardando_pagamento', }), ...(appliedCoupon && { cupomAplicado: { id: appliedCoupon.id, codigo: appliedCoupon.codigo, tipoDesconto: appliedCoupon.tipoDesconto, valorDesconto: appliedCoupon.valorDesconto, descontoCalculado: discountAmount } }) }; try { if (appliedCoupon) { await runTransaction(db, async (transaction) => { const couponRef = doc(db, 'cupons', appliedCoupon.id); const userCouponUsageRef = doc(db, 'clientes', currentUser.uid, 'couponUsage', couponData.id); const couponSnap = await transaction.get(couponRef); const userCouponUsageSnap = await transaction.get(userCouponUsageRef); if (!couponSnap.exists()) { throw new Error("Cupom não existe mais!"); } const currentUsosAtuais = couponSnap.data().usosAtuais || 0; if (couponSnap.data().usosMaximos !== null && currentUsosAtuais >= couponSnap.data().usosMaximos) { throw new Error("Cupom já atingiu o limite total de usos."); } let currentUserUses = 0; if (appliedCoupon.usosPorUsuario !== null) { if (userCouponUsageSnap.exists()) { currentUserUses = userCouponUsageSnap.data().count || 0; } if (currentUserUses >= appliedCoupon.usosPorUsuario) { throw new Error("Você já usou este cupom o número máximo de vezes."); } } transaction.update(couponRef, { usosAtuais: currentUsosAtuais + 1 }); if (appliedCoupon.usosPorUsuario !== null) { transaction.set(userCouponUsageRef, { count: currentUserUses + 1 }, { merge: true }); } }); } const docRef = await addDoc(collection(db, 'pedidos'), pedido); setConfirmedOrderDetails({ id: docRef.id, cliente: pedido.cliente, itens: pedido.itens, subtotal: subtotalCalculado, taxaEntrega: taxaAplicada, totalFinal: finalOrderTotal, formaPagamento: formaPagamento, trocoPara: valorTrocoPara, tipoEntrega: pedido.tipoEntrega, cupomAplicado: appliedCoupon ? { codigo: appliedCoupon.codigo, desconto: discountAmount } : null }); setShowOrderConfirmationModal(true); toast.success('Seu pedido foi enviado com sucesso! 🎉'); setCarrinho([]); setFormaPagamento(''); setTrocoPara(''); setTaxaEntregaCalculada(0); setBairroNaoEncontrado(false); setCouponCodeInput(''); setAppliedCoupon(null); setDiscountAmount(0); } catch (error) { console.error("Erro ao enviar pedido ou aplicar cupom (transação): ", error); if (error.message && (error.message.includes("limite total de usos") || error.message.includes("máximo de vezes") || error.message.includes("Cupom não existe mais"))) { toast.error(`❌ Erro no cupom: ${error.message}`); } else { toast.error(`❌ Ocorreu um erro ao enviar seu pedido. Por favor, tente novamente.`); } } };
  const handleLoginModal = async (e) => { e.preventDefault(); setErrorAuthModal(''); try { await signInWithEmailAndPassword(auth, emailAuthModal, passwordAuthModal); toast.success('Login realizado com sucesso!'); setShowLoginPrompt(false); setIsRegisteringInModal(false); setEmailAuthModal(''); setPasswordAuthModal(''); setErrorAuthModal(''); setNomeAuthModal(''); setTelefoneAuthModal(''); setRuaAuthModal(''); setNumeroAuthModal(''); setBairroAuthModal(''); setCidadeAuthModal(''); setComplementoAuthModal(''); } catch (error) { let msg = "Erro no login. Verifique suas credenciais."; if (error.code === 'auth/user-not-found') msg = "Usuário não encontrado. Crie uma conta."; else if (error.code === 'auth/wrong-password') msg = "Senha incorreta."; else if (error.code === 'auth/invalid-email') msg = "Email inválido."; setErrorAuthModal(msg); toast.error(msg); console.error("Login error:", error); } };
  const handleRegisterModal = async (e) => { e.preventDefault(); setErrorAuthModal(''); if (!nomeAuthModal.trim() || !telefoneAuthModal.trim() || !emailAuthModal.trim() || !passwordAuthModal.trim() || !ruaAuthModal.trim() || !numeroAuthModal.trim() || !bairroAuthModal.trim() || !cidadeAuthModal.trim()) { setErrorAuthModal('Por favor, preencha todos os campos obrigatórios, incluindo o endereço completo.'); toast.error('Por favor, preencha todos os campos obrigatórios, incluindo o endereço completo.'); return; } try { const userCredential = await createUserWithEmailAndPassword(auth, emailAuthModal, passwordAuthModal); const user = userCredential.user; await setDocFirestore(doc(db, 'clientes', user.uid), { nome: nomeAuthModal.trim(), telefone: telefoneAuthModal.trim(), email: emailAuthModal.trim(), endereco: { rua: ruaAuthModal.trim(), numero: numeroAuthModal.trim(), bairro: bairroAuthModal.trim(), cidade: cidadeAuthModal.trim(), complemento: complementoAuthModal.trim() || null }, criadoEm: Timestamp.now(), }); toast.success('Cadastro realizado com sucesso! Você está logado.'); setShowLoginPrompt(false); setIsRegisteringInModal(false); setEmailAuthModal(''); setPasswordAuthModal(''); setNomeAuthModal(''); setTelefoneAuthModal(''); setRuaAuthModal(''); setNumeroAuthModal(''); setBairroAuthModal(''); setCidadeAuthModal(''); setComplementoAuthModal(''); setErrorAuthModal(''); } catch (error) { let msg = "Erro no cadastro. Tente novamente."; if (error.code === 'auth/email-already-in-use') msg = "Este email já está cadastrado."; else if (error.code === 'auth/weak-password') msg = "Senha muito fraca (mín. 6 caracteres)."; setErrorAuthModal(msg); toast.error(msg); console.error("Registration error:", error); } };
  
  if (authLoading) { return (<div className="flex justify-center items-center h-screen bg-white"> <p className="text-[var(--marrom-escuro)]">Verificando status de login...</p> </div>); }
  if (isAdmin || isMasterAdmin) { useEffect(() => { toast.error('Acesso negado. Esta página é exclusiva para clientes. Redirecionando...'); const timer = setTimeout(() => { if (isMasterAdmin) { navigate('/master-dashboard'); } else if (isAdmin) { navigate('/painel'); } else { navigate('/'); } }, 1500); return () => clearTimeout(timer); }, [isAdmin, isMasterAdmin, navigate]); return (<div className="flex justify-center items-center h-screen bg-gray-100"> <div className="text-center p-8 bg-white rounded-lg shadow-md"> <h2 className="text-2xl font-bold text-red-600 mb-4">Acesso Negado</h2> <p className="text-gray-700">Esta área é exclusiva para clientes. Você será redirecionado.</p> </div> </div>); }
                ...
                <div className="text-center mt-6">
                  {todosItensVisiveis ? (
                    <button
                      ...
                      // Estilo do botão "Ver menos" (pode manter o cinza ou alterar também)
...
<button
    ...
    // Estilo com borda e texto na cor principal, com preenchimento no hover
    className="border-2 border-[var(--vermelho-principal)] text-[var(--vermelho-principal)] font-semibold py-2 px-6 rounded-lg hover:bg-[var(--vermelho-principal)] hover:text-blue transition-colors duration-300"
>
    Ver mais
</button>
                  )}
