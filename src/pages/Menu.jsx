    ...
    const fetchEstabelecimentoAndCardapio = async () => {
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          if (!data.ativo) { // Verifica se o estabelecimento está ativo
          ...
          // Aplica filtro de categoria e busca
    ...
    try {
      if (appliedCoupon) {
        await runTransaction(db, async (transaction) => {
          ...
          const userCouponUsageRef = doc(db, 'clientes', currentUser.uid, 'couponUsage', appliedCoupon.id); // Corrigido couponData.id para appliedCoupon.id
                ...
                <div className="text-center mt-6">
                  {todosItensVisiveis ? (
                    ...
                    <button
                        ...
                        className="border-2 border-[var(--vermelho-principal)] text-[var(--vermelho-principal)] font-semibold py-2 px-6 rounded-lg hover:bg-[var(--vermelho-principal)] hover:text-white transition-colors duration-300" // Corrigido hover:text-white
                    >
                        Ver mais
                    </button>
                  )}
