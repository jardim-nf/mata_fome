...
function Menu() {
  ...
  useEffect(() => {
    ...
    const fetchEstabelecimentoAndCardapio = async () => {
      try {
        ...
        setNomeEstabelecimento("Erro ao carregar");
        setAllProdutos([]);
      }
    };
  ...
  // --- LÓGICA DE RENDERIZAÇÃO E REDIRECIONAMENTO ---
  if (authLoading) {
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto pb-48 md:pb-0">
      ...
      {/* MODAL DE LOGIN/CADASTRO CORRIGIDO */}
      {showLoginPrompt && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-[1000]">
            <div className="bg-white rounded-lg shadow-xl p-6 max-w-sm w-full relative text-center">
                <button 
                    onClick={() => { setShowLoginPrompt(false); /* Adicione a limpeza de outros estados se necessário */ }} 
